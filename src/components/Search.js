...
const Search = () => {
  ...
  // Perform search when searchTerm or searchType changes
  useEffect(() => {
    if (!searchTerm.trim()) {
      setSearchResults([]);
    ...
    if (searchType === 'all' || searchType === 'posts') {
      // Search in post text
      const textMatches = allPosts.filter(post => 
        post.text && post.text.toLowerCase().includes(term)
      );
      filteredPosts = [...filteredPosts, ...textMatches];
    }

    if (searchType === 'all' || searchType === 'users') {
      // Search by user email
      const userMatches = allPosts.filter(post => 
        post.user && post.user.toLowerCase().includes(term)
      );
      filteredPosts = [...filteredPosts, ...userMatches];
    }
    ...
    setIsSearching(false);
  }, [searchTerm, searchType, allPosts]);
  ...
  const highlightSearchTerm = (text, term) => {
    ...
    return text.replace(regex, '<mark style="background-color: #ffeb3b; font-weight: bold;">$1</mark>');
  };

  return (
    <div style={{ maxWidth: '600px', margin: '20px auto', padding: '20px' }}>
      <h3>🔍 Search Posts and Users</h3>
          ...
          {searchResults.length === 0 ? (
            ...
            searchResults.map((post) => (
              ...
              >
                <div style={{ marginBottom: '10px' }}>
                  <strong 
                    ...
                    dangerouslySetInnerHTML={{
                      __html: highlightSearchTerm(post.user, searchTerm)
                    }}
                  />
                ...
                {/* Text content */}
                <p 
                  style={{ marginBottom: '15px', lineHeight: '1.5' }}
                  dangerouslySetInnerHTML={{
                    __html: highlightSearchTerm(post.text, searchTerm)
                  }}
                />
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                  ...
                  >
                    💬 Reply ({allReplies[post.id]?.length || 0})
                  </button>
                </div>
                ...
                {/* Display Replies */}
                {allReplies[post.id] && allReplies[post.id].length > 0 && (
                  <div style={{ marginTop: '15px', borderTop: '1px solid #eee', paddingTop: '15px' }}>
                    <h4 style={{ margin: '0 0 10px 0', fontSize: '16px', color: '#666' }}>
                      Replies ({allReplies[post.id].length})
                    </h4>
                    {allReplies[post.id].map((reply) => (
                      <div
                        key={reply.id}
                        style={{
                          marginLeft: '20px',
      ...
      {!searchTerm && (
        <div style={{
          ...
          <h4 style={{ margin: '0 0 10px 0', color: '#333' }}>How to search:</h4>
          <ul style={{ margin: '0', paddingLeft: '20px', color: '#666' }}>
            <li>Type any text to search in post content</li>
            <li>Type a user's email to find their posts</li>
            <li>Use the filter options to search only posts or users</li>
            <li>Search results are highlighted for easy identification</li>
          </ul>
        </div>
      )}
    </div>
